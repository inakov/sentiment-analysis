...
//  val sessions = groupByUser.filter(_._2.size > 5).sortBy(-_._2.size).filter(check => mean[Int](check._2.map(_._4.toInt)) < -67).map(_._2.map(check => (check._1, check._4.toInt)))
//
...
//
//  val firstSession = sessions.take(58).last.toList
...
//  points.map(geometricMean(_)).foreach(println)
  //.foreach(println)
//  val time = firstSession.map(_._1).toList
//  println(time.last - time.head)
